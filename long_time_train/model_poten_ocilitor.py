...
os.makedirs('./model_para_no_sym', exist_ok=True)
...
if __name__ == '__main__':
    ...
    N = 16000   # number of interior points # 对时间成本来说几乎是平方量级
    h :float= L / (N+1)
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer,patience=40,threshold=1e-4)
    loss_fn=nn.L1Loss()
    ...
    for i in range(epoch):
        ...
        if (i+1)%100==0:torch.save(model.state_dict(),f'./model_para_no_sym/model_para_{lr}_{en_num}.pth')
        scheduler.step(loss)
        if optimizer.param_groups[0]["lr"] <= 1.1e-8:break
    print('terminal epoch: ',i+1)
    torch.save(model.state_dict(),f'./model_para_no_sym/model_para_{lr}_{en_num}.pth')